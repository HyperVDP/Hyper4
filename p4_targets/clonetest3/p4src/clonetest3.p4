...
action do_mod_meta(value) { modify_field(meta.egress, value); }
table mod_meta {
  actions {
    do_mod_meta;
  }
...
action do_forward(port) { modify_field(standard_metadata.egress_spec, port); }
table forward {
  reads {
...
action do_finalize() { modify_field(extra.egress, meta.egress); }
